...
import { ExternalLink, X, Info, Lock, Star, Download } from 'lucide-react';
...
export const SearchLoadingModal: React.FC<SearchLoadingModalProps> = ({
  ...
  useEffect(() => {
    ...
    // Auto finish after totalDisplayTime
    const autoFinishTimer = setTimeout(() => {
      onCompleteRef.current();
    }, totalDisplayTime * 1000);

    // Skip unlock timer (e.g. compulsory 5 seconds view time)
  ...
  return (
    <div className="fixed inset-0 z-[100] w-screen h-[100dvh] bg-[#1a1a1a] text-white flex flex-col justify-between overflow-hidden select-none animate-in fade-in duration-300">
      
      {/* 1. TOP GOOGLE ADMOB / ADSENSE HEADER BAR */}
      <div className="p-3 sm:p-4 flex items-center justify-between bg-[#242424] border-b border-[#333333] z-20">
        
        {/* COMPULSORY SKIP TIMER LOCK OR UNLOCKED CLOSE BUTTON */}
        <div className="flex items-center gap-2">
          {canSkip ? (
            <button
              onClick={onComplete}
              className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs sm:text-sm flex items-center gap-1.5 transition-all shadow-lg hover:scale-105 cursor-pointer"
            ...
            <div className="px-3.5 py-1.5 rounded-xl bg-slate-900 border border-slate-700 text-amber-400 text-xs font-bold flex items-center gap-2 shadow-inner">
              <Lock className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
              <span>Skip in <strong className="text-white font-mono text-sm">{skipTimer}s</strong></span>
            </div>
          )}
        </div>
        ...
        {/* TRANSIT SEARCH INDICATOR */}
        <div className="hidden md:flex flex-col items-end text-[11px] text-slate-400 font-mono">
          <span>Searching: {from} → {to}</span>
          <span className="text-emerald-400 font-bold">Ad Auto-Finish in {secondsRemaining}s</span>
        </div>
      </div>

      {/* 2. CENTER HIGH-IMPACT MEDIA CANVAS (SUPPORTING VIDEO OR POSTER) */}
      <div className="relative flex-1 w-full bg-[#121212] flex items-center justify-center p-2 sm:p-4 overflow-hidden">
        {ad?.mediaType === 'video' && ad?.videoUrl ? (
          <div className="relative w-full h-full max-h-[65vh] sm:max-h-[75vh] rounded-2xl overflow-hidden bg-black border border-[#333]">
            <video
              src={ad.videoUrl}
              autoPlay
              muted
          ...
          <div className="relative w-full h-full max-h-[65vh] sm:max-h-[75vh] rounded-2xl overflow-hidden bg-[#202020] border border-[#333] shadow-2xl">
            <Image
              src={ad?.image || 'https://images.unsplash.com/photo-1555507036-ab1f4038808a?q=80&w=1600&auto=format&fit=crop'}
              alt={ad?.title || 'Google AdMob Interstitial'}
              fill
      ...
      <div className="p-4 sm:p-5 bg-[#242424] border-t border-[#333333] space-y-3 relative z-20">
        ...
        {/* LARGE METALLIC / ROUNDED GOOGLE AD CTA BUTTON */}
        <div className="pt-1 flex flex-col items-center gap-2">
          <a
            ...
            className="w-full sm:max-w-md h-13 rounded-xl bg-gradient-to-b from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-extrabold text-sm border border-blue-500 shadow-xl flex items-center justify-center gap-2 transition-transform active:scale-98 cursor-pointer uppercase tracking-wider"
          >
        ...
        {/* GOOGLE ADCHOICES ICON BOTTOM LEFT */}
